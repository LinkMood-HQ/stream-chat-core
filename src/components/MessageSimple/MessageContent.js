...
import { MessageStatus } from 'stream-chat-react-native-core';
...
const ContainerInner = styled.View`
  display: flex;
  background-color: ${({ groupStyle }) =>
    groupStyle.indexOf('left') !== -1 ? '#ECECEC' : '#ff0b5a'};
  flex-direction: row;
...
const MetaText = styled.Text`
  font-size: 14;
  margin-top: ${({ alignment }) => (alignment === 'left' ? '13' : '3')};
  color: ${({ alignment }) => (alignment === 'left' ? '#CCCCCC' : '#FFABC6')};
...
class MessageContent extends React.PureComponent {
  ...
  static propTypes = {
    ...
    onMessageInfoSelect: PropTypes.func,
    /**
  ...
  openMessageInfo = () => {
    if (this.props.onMessageInfoSelect)
      this.props.onMessageInfoSelect(this.props.message);
  };
  ...
  render() {
    ...
    if (
      messageActions &&
      messageActions.indexOf(MESSAGE_ACTIONS.info) > -1 &&
      isMyMessage(message)
    )
      options.splice(1, 0, {
        id: MESSAGE_ACTIONS.info,
        title: t('Info'),
      });
                    ...
                    >
                      <MetaText alignment={alignment}>
                        {this.props.formatDate
                          ...
                          : tDateTimeParser(message.created_at).format('H:mm')}
                      </MetaText>
                      {isMyMessage(message) && (
            ...
            {!hasAttachment && (
              <MetaContainer>
                ...
                >
                  ...
                  {isMyMessage(message) && <MessageStatus {...this.props} />}
                </View>
              </MetaContainer>
            )}
          </ContainerInner>
          ...
          {/* {!MessageFooter && showTime ? (
            <MetaContainer>
              <View style={{ flexDirection: 'row' }}>
                ...
                {isMyMessage && <MessageStatus {...this.props} />}
              </View>
            </MetaContainer>
          ) : null} */}
          <ActionSheet
